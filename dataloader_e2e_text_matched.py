...
def pad_collate(batch):
    ...
    speech_padding_mask = torch.ones((B, torch.max(segment_len))).bool()
    speech_padding_mask[torch.arange(B).unsqueeze(1), dont_skip] = False
    
    # text_attn_mask = torch.ones((T, B)).dtype(bool)
    # text_attn_mask[torch.arange(T).unsqueeze(1), dont_skip] = False
    # target_len = torch.full(B, fill_value = num_Q_per_segment)
    return {
        "B": B,
        "T": T,
