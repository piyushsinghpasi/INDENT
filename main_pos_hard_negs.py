...
# dataloader_pos_hard_negs_new --> more positives are not matched but, sequential
from dataloader_pos_hard_negs_new import *
from utils.plot_graphs import plot_chunks
from nets.network_old import Feat_Merger
...
class customContrastiveLoss(nn.Module):
    def __init__(self, temperature = 1.0, alpha = 1., beta = 1.0):
    ...
    def forward(self, s, t, NS, NT, PS, PT):
        ...
        num_t_s = numerator*self.alpha + numerator_PS*self.beta
        num_s_t = numerator*self.alpha + numerator_PT*self.beta
...
def train(args, model, train_loader, optimizer, criterion, epoch, log_file=None, hid_dim=768):
    ...
    running_L_speech_to_text, running_L_text_to_speech, running_similarity_loss, running_loss = 0., 0., 0., 0.
    for batch_idx, sample in enumerate(train_loader):
        ...
        running_L_speech_to_text += L_speech_to_text.item()
        running_L_text_to_speech += L_text_to_speech.item()
        ...
        if batch_idx % args.log_interval == 0:
            out_loss = 'Train Epoch: {} [{}/{} ({:.0f}%)]\tL_speech_to_text:{:.3f}\tL_text_to_speech:{:.3}\tsimilarity_loss:{:.3f}\tLoss:{:.3f}'.format(
                epoch, batch_idx*s_feat.shape[0], len(train_loader.dataset),
                       100. * batch_idx / len(train_loader), running_L_speech_to_text/(batch_idx+1), running_L_text_to_speech /(batch_idx+1), running_similarity_loss/(batch_idx+1), running_loss/(batch_idx+1))
            print(out_loss)
    if log_file is not None:
        with open(log_file,'a') as f:
            f.write("{:.6f}\t{:.6f}\t{:.6f}\t{:.6f}\n".format(running_L_speech_to_text/(batch_idx+1), running_L_text_to_speech /(batch_idx+1), running_similarity_loss/(batch_idx+1), running_loss/(batch_idx+1)))
...
def eval(
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(chunk_loader):
            print(sample.keys())
            chunks, seg_id = sample['chunks'], sample['segment_global_id'][0]
        ...
        for batch_idx, sample in enumerate(question_loader):
            segment_scores = dict()
            Question_feat, GT_seg_id = sample['Question'].to(DEVICE), (sample['segment_global_id'])[0]
...
def main():
    ...
    parser.add_argument(
        "--checkpoint", type=str, help="save model name")
    parser.add_argument(
    ...
    if args.mode == 'train':
        ...
        # tune temperature
        criterion = customContrastiveLoss(temperature = 1.0, alpha=1.0, beta = 0.)
