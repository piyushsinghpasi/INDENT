...
def pad_collate(batch):
    ...
    for sample_dict in batch:
        ...
        # print(sample_dict['idx'], (torch.cat(NT, dim=0)).shape)
...
class CARE_dataset(Dataset):
    ...
    def __init__(self, segs_dir, matchings = 'matched_dict_sim_partition.pkl', qns_file = './../care_india/questions/questions_00-02_hindi.json', qns_emb_file = './../care_india/questions/questions_00-02_hindi_labse_embeddings.json', 
    train=None, transform=None, k=0, nsample = 10, model=None, hard_neg_thres = 0.5):
        ...
        for segs in seg_audio_files:
            ...
            for ind, chunk in enumerate(chunk_files):
                ...
                chunks_list += [{'path': chunk, 'audio': audio, 'segment_global_id': l[-1], 'seg_ind': seg_ind, 'chunk_ind': ind, 'chunk_feat': np.load(feat_file)}]
            ...
            for chunk in matched_chunks.keys():
                ...
                feat_file = chunk_path.replace("vad_chunks", "vad_chunks_feat").replace("wav", "npy")
                # print(chunk_path)
                if not os.path.exists(feat_file): continue
                # print(chunk_path)
                # matched_list += [{'chunk_list_idx' : tmp_name_path_idx[chunk_path], 'qn_no': qn, 'qn_text' : qn_text, 'qn_emb': qn_emb}]
                matched_list += [{'chunk_list_idx' : path_idx[chunk_path], 'qn_no': qn, 'qn_text' : qn_text, 'qn_emb': qn_emb, 'chunk_path':chunk_path}]

        # with open('../care_india_data/train_chunks_list.pkl', 'rb') as f:
        #     self.chunks_list = pickle.load( f)
        
        # with open('../care_india_data/train_matched_list.pkl', 'rb') as f:
        #     self.matched_list = pickle.load(f)
            
        # with open('../care_india_data/train_path_idx.pkl', 'rb') as f:
        #     self.path_idx = pickle.load(f)

        self.qns_emb = qns_emb
        self.model = model
        self.hard_neg_thres = hard_neg_thres
        
        # with open('../care_india_data/train_chunks_list.pkl', 'wb') as f:
        #     pickle.dump(self.chunks_list, f)
        
        # with open('../care_india_data/train_matched_list.pkl', 'wb') as f:
        #     pickle.dump(self.matched_list, f)
            
        # with open('../care_india_data/train_path_idx.pkl', 'wb') as f:
        #     pickle.dump(self.path_idx, f)
    ...
    def __getitem__(self, idx):  # Negatives are matched samples - (s_i, t_i) - outside [-2k, 2k]
        ...
        ../care_india/annotations/audio_hindi_train_segments/AA_0b85442c-00b8-45bb-bbda-499ba0c2d8ca_AFTER_0S___4/vad_chunks/chunk-005.wav torch.Size([2, 768])
        '''
        ...
        while chunk_list_idx+offset < len(chunks_list) and chunks_list[chunk_list_idx]['audio'] == audio:
            more_pos_s_right += [chunks_list[chunk_list_idx+offset]['chunk_feat']]
            offset += 1

        offset = 1
        while chunk_list_idx-offset >= 0 and chunks_list[chunk_list_idx]['audio'] == audio:
            more_pos_s_left = [chunks_list[chunk_list_idx-offset]['chunk_feat']] + more_pos_s_left
            offset += 1
        ...
        more_pos_t_left = [self.qns_emb[qn] for qn in qns_list[qns_list_ind-k:qns_list_ind]]
        more_pos_t_right = [self.qns_emb[qn] for qn in qns_list[qns_list_ind:qns_list_ind+k]]

        more_pos_t = more_pos_t_left + more_pos_t_right

        inds_s = random.sample(range(len(more_pos_s)), self.k)
        inds_t = random.sample(range(len(more_pos_t)), self.k)

        # neg_s_sampled += [neg_s_random[i] for i in inds_s]
        # neg_t_sampled += [neg_t_random[i] for i in inds_t]

        
        # pos_inds = random.sample(range(len(more_pos_s)), self.k)
        pos_s_sampled = [more_pos_s[i] for i in inds_s]
        pos_t_sampled = [more_pos_t[i] for i in inds_t]
        ...
        if self.model is None:
            ...
            self.model.eval()
            with torch.no_grad(): 
                nsample_copy = self.nsample
                max_iter = 1000
                # print("hard negatives !!!!")

                neg_s_sampled = []
                neg_t_sampled = []
                while(max_iter and nsample_copy):
                    max_iter -= 1
                    ind = random.sample(range(len(more_pos_s)), 1)[0]
                    C_j = torch.from_numpy(neg_s[ind]).to(DEVICE_dataloader)
                    C_j_feat, _ = self.model(C_j, None)
                    Q_i = pos_t.to(DEVICE_dataloader)
                    _, Q_i_feat = self.model(None, Q_i)
                    curr_score = (Q_i_feat * C_j_feat).sum(dim=-1).item()
                    if (curr_score >= self.hard_neg_thres):
                        neg_s_sampled += [neg_s[ind]]
                        neg_t_sampled += [neg_t[ind]]
                        nsample_copy -= 1
                    
                if (len(neg_s_sampled) < self.nsample):
                    remaining_sample = self.nsample - len(neg_s_sampled)
                    inds = random.sample(range(len(neg_s)), min(remaining_sample, len(neg_s)))
                    neg_s_sampled += [neg_s[i] for i in inds]
                    neg_t_sampled += [neg_t[i] for i in inds]
        ...
        if len(neg_s_sampled) < self.nsample:
            ...
            qns_list = list(self.qns_emb.keys())
            qns_list_ind = qns_list.index(qn)
            neg_t_left = [self.qns_emb[qn] for qn in qns_list[:qns_list_ind-k]]
            neg_t_right = [self.qns_emb[qn] for qn in qns_list[qns_list_ind+k:]]

            neg_s_random = neg_s_left + neg_s_right
            neg_t_random = neg_t_left + neg_t_right

            inds_s = random.sample(range(len(neg_s_random)), self.nsample - len(neg_s_sampled))
            inds_t = random.sample(range(len(neg_t_random)), self.nsample - len(neg_t_sampled))
        ...
        sample = { 'pos_s': pos_s, 'pos_t': pos_t, 'NS': neg_s_sampled, 'NT': neg_t_sampled, 'PS' : pos_s_sampled, 'PT' : pos_t_sampled,
            'idx':matched_list[idx]['chunk_path'],
         }
...
class Test_dataset(Dataset):
    def __init__(self, segs_dir, file_name, transform = None):
        ...
        for segs in seg_audio_files:
            ...
            segments_list += [{'file': file_name, 'segment_id': seg_ind, 'segment_global_id': l[-1], 'chunks': chunk_emb_list}]
    ...
    def __init__(self, file_name, test_file='./../care_india/audio_features_dict_valdata_5ques.json', qns_file = './../care_india/questions/questions_00-02_hindi.json', qns_emb_file = './../care_india/questions/questions_00-02_hindi_labse_embeddings.json', transform=None):
        ...
        for seg_ind in curr_segs.keys():
            for qn in curr_segs[seg_ind]['questions']:
                qns_list += [{'qn_no': qn, 'segment_id': seg_ind, 'segment_global_id': file_name, 'Question': qns_emb[qn]}]
