...
def pad_collate(batch):
    ...
    grp_count = 0
    M = 0
    for _, grp in enumerate(batch):
        for _, seg in enumerate(grp):
            M = max(M, len(seg['chunks']))
    ...
    # print((speech_padding_mask[:,:, :,0].squeeze().sum(-1) == 0).sum(-1))
    ...
    def __init__(self, segs_dir, file_name, num_chunks, transform = None):
        ...
        for segs in seg_audio_files:
            ...
            for chunk in chunk_files:
                ...
                if num%num_chunks == 0:
                    fixed_chunks_list += [{'file': file_name, 'chunks': chunk_emb_list, 'chunk_timestep_len': chunk_timestep_len}]
                    chunk_emb_list = []
                    chunk_timestep_len = []
