...
from dataloader_e2e_text_grped import *
# from dataloader_e2e_speech_batched import *
# from nets.net_e2e_indicspeech import Feat_Merger
from nets.net_e2e_text import Feat_Merger
from utils.recall_eval import eval

from utils.CE_style_loss import CE_style
...
def train_test(args, model, train_loader, optimizer, criterion, epoch, N, log_file=None, hid_dim=768):
    model.load_state_dict(torch.load("./models/" + "deep-speech-B4-E40-lr3e-4-N4-cross-cont-only" + ".pt"))
    model.eval()
    with torch.no_grad():
        running_loss, running_cont_loss, running_sim_loss, running_cont_loss_self, running_sim_loss_self = 0., 0., 0., 0., 0.
        for batch_idx, sample in enumerate(train_loader):
            ...
            cont_loss, sim_loss = criterion(self_attd_chunk, cross_attd_chunk, speech_padding_mask, valid_negs_mask, sampled_neg_inds)
            cont_loss_self, sim_loss_self = criterion(self_attd_chunk, self_attd_chunk, speech_padding_mask, valid_negs_mask, sampled_neg_inds)
            
            alpha, beta = 0.8, 1.
            loss = cont_loss #+ beta*(sim_loss)
            loss.backward()
            optimizer.step()
            ...
            if (batch_idx % args.log_interval) == 0:
                out_loss = 'Train Epoch: {} [{}/{} ({:.0f}%)]\tCont.Loss:{:.3f}\tSim Loss:{:.3f}\tCont.Self:{:.3f}\tSim Self:{:.3f}\tloss:{:.3f}'.format(
                    epoch, batch_idx*self_attd_chunk.shape[0], len(train_loader.dataset),
                        100. * batch_idx / len(train_loader), running_cont_loss/(batch_idx+1), running_sim_loss/(batch_idx+1), running_cont_loss_self/(batch_idx+1), running_sim_loss_self/(batch_idx+1), running_loss/(batch_idx+1))
                print(out_loss)
        if log_file is not None:
            with open(log_file,'a') as f:
                f.write("{:.6f}\t{:.6f}\t{:.6f}\t{:.6f}\t{:.6f}\n".format(running_cont_loss/(batch_idx+1), running_sim_loss/(batch_idx+1), running_cont_loss_self/(batch_idx+1), running_sim_loss_self/(batch_idx+1), running_loss/(batch_idx+1)))
...
def train(args, model, train_loader, optimizer, criterion, epoch, N, log_file=None, hid_dim=768):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        alpha, beta = 0.8, 1e-1
        loss = cont_loss# + cont_loss_self # + beta*(sim_loss + sim_loss_self)
        loss.backward()
        optimizer.step()
...
def main():
    ...
    if args.mode == 'train':
        ...
        with open(args.result_logging_file_path+"{}_train_loss.txt".format(args.checkpoint),'w') as logfile:
            logfile.write("Cont.Loss\tSimLoss\tCont.SelfLoss\tSimSelfLoss\tLoss\n")
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            for file_name in train_files:
                ...
                question_dataset = Question_dataset_old(
                    file_name=file_name, 
            ...
            for file_name in val_files:
                chunk_dataset = Test_dataset_fixed(
                    segs_dir = args.val_segs_dir, 
                    file_name=file_name, 
                    num_chunks=args.window_size,
                    # transform = transforms.Compose([ToTensorChunk()])
                )
                question_dataset = Question_dataset_Fixed(
                    file_name=file_name, 
                    test_data_object=chunk_dataset
                )
                chunk_loader = DataLoader(
