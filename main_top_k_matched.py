...
from dataloader_batch_new import *
...
class customContrastiveLoss(nn.Module):
    ...
    def forward(self, s, t, NS, NT):
        ...
        log_t_s = torch.mean(torch.log(numerator/denominator_NS), dim=0)
        log_s_t = torch.mean(torch.log(numerator/denominator_NT), dim=0)
...
def train(args, model, train_loader, optimizer, criterion, epoch, log_file=None, hid_dim=768):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        # exit(0)
        batch_size, nsample, hdim = sample["batch_size"], sample["nsample"], batch_NS_pad.shape[-1]
        ...
        L_speech_to_text, L_text_to_speech, similarity_loss = criterion(s_feat, t_feat, batch_unpack_NS_feat, batch_pack_NT_feat)
        ...
        for batch_idx, sample in enumerate(question_loader):
            ...
            for seg_id, chunk_feat in seg_chunk_feat.items():
                ...
                if (len(chunk_scores.size()) == 0): 
                    chunk_scores = torch.Tensor([chunk_scores.item()]).to(DEVICE)
                    # print("hhh", chunk_scores)
            ...
            if (show_plots):
                ...
                for seg_id in all_scores[:5]:
                    ...
                    tmp_Q_text = [qns_dict[q] for q in seg_to_Q[seg_id]['questions']]
                ...
                plot_chunks(
                    ...
                    Q_text,
                    top_5_text,
                    GT_score = segment_scores[GT_seg_id],
                    top_5_scores = [(i,segment_scores[i]) for i in top_5_scores],
