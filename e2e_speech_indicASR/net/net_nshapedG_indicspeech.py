...
class Gaussian_cross_attn(nn.Module):
    ...
    def generate_weights(self, num_chunks, max_chunk, curr_device, scale=0.5):
    ...
    def forward(self, segment_len, max_chunk, Q_emb, scale=0.5):
        batch_wts = []
        for num_chunk in segment_len:
            wts = self.generate_weights(num_chunk.item(), max_chunk, Q_emb.device, scale).unsqueeze(0)
...
class Feat_Merger(nn.Module):
    ...
    def __init__(self, input_dim = 768, hidden_dim = 768, dropout = 0.3, kernel_size = 20, stride = 2):
    ...
    def forward(self, speech, text, speech_padding_mask, segment_len, chunk_timestep_len, max_pad, speech_attn_mask = None, padding_cross_attn_mask = None, scale=0.5):
        '''
        ...
        cross_attd_chunk = self.gaussian_cross_attn_layer(segment_len, self_attd_chunk.size()[1], t_feat, scale)
