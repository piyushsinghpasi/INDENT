'''
This file contain main code to train and test INDENT model. (see argument parser for options)
It has support for inference in many languages on question front (see argument parser --lang argument)
'''
...
from net.net_e2e_indicspeech import Feat_Merger
...
def main():
    ...
    parser.add_argument(
        '--K', type=int, default=0, help='sets range [-2K, 2K] to be excluded for sampling negatives (not useful anymore as negatives are dynamically sampled, see section 4.3 Dynamic In-audio negative sampling https://www.ijcai.org/proceedings/2023/0683.pdf)')
    parser.add_argument(
        '--nsample', type=int, default=10, help='no. of negatives per chunk (not useful anymore as negatives are dynamically sampled, see section 4.3 Dynamic In-audio negative sampling https://www.ijcai.org/proceedings/2023/0683.pdf)')
    parser.add_argument(
    ...
    parser.add_argument(
        '--D', type=int, help='D Times Data is seen, i.e D-1 time Augmented, see last few lines in section 4.3 Dynamic In-audio negative sampling https://www.ijcai.org/proceedings/2023/0683.pdf'
    )
    parser.add_argument(
        '--std', type=float, help='standard deviation in Gaussian Layer', default=0.5,
    )
    parser.add_argument(
        '--lang', type=str, help='langauge code of translated Q. labse embeddings dict',
    )
    ...
    # slightly different (1 Linear layer extra for IndicASR to project down 1024-dim features to 768-dim. features) model architecture is used for IndicASR vs Vakyansh feature extractor (Both are Wav2Vec2 model finetuned on Hindi language, ASR task)
    # due to difference is feature dim 1024 vs 768 resp.
    model = Feat_Merger(input_dim = 1024, hidden_dim = 768, std=args.std).to(DEVICE)

    vad_chunks_feat_dir = None
    # features are extracted from central layers of each model(layer no. mentioned below)
    # features are extracted after VAD i.e chunk level features
    # utils has code for feature extraction
    if args.feat_model == "vakyansh":
        vad_chunks_feat_dir = "vad_chunks_feat_vakyansh_layer6_final_norm" 
    elif args.feat_model == "indicASR":
        vad_chunks_feat_dir = "vad_chunks_feat_indicASR_layer15_final_norm"
    elif args.feat_model == "xlsr":
        vad_chunks_feat_dir = "vad_chunks_feat_xlsr_layer15_final_norm"
    else:
        raise ValueError("Unknown model feature extractor: Pls pick from vakyansh, indicASR or xlsr")

    if args.mode == 'train':
        train_dataset = CARE_dataset(
            segs_dir = args.train_segs_dir, 
            nsample = args.nsample,
            vad_chunks_feat_dir = vad_chunks_feat_dir,
            D = args.D,
        )
        ...
        for f in os.listdir(args.val_segs_dir) :
            if os.path.isdir(os.path.join(args.val_segs_dir,f)):
                filename = f.split("___")[0] # file naming system was filename___segmentID
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            for file_name in train_files:
                ...
                # we skipped evaluation of training set
                a, b, c, d = 0, 0, 0, 0
                # a, b, c, d = eval(
                #     model, 
                #     question_loader, 
