'''
This file contains code for training and evaluating for model with variable standard deviation in gaussian weighted cross attention model inside the architecture (net.net_nshapedG_indicspeech import Feat_Merger)
This is just part of experiments done to find best architecture.
Main code is main_hindi_fixed_trans.py
'''
...
from net.net_nshapedG_indicspeech import Feat_Merger
...
def train(args, model, train_loader, optimizer, criterion, epoch, N, scale=0.5, log_file=None, hid_dim=768):
    model.train()
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        self_attd_chunk, cross_attd_chunk = model(chunks_feat, Q_emb, speech_padding_mask, segment_len, chunk_timestep_len, M, speech_attn_mask, padding_cross_attn_mask, scale)
...
def main():
    ...
    parser.add_argument(
        '--K', type=int, default=0, help='sets range [-2K, 2K] to be excluded for sampling negatives (deprecated)')
    parser.add_argument(
        '--nsample', type=int, default=10, help='no. of negatives (deprecated)')
    parser.add_argument(
    ...
    parser.add_argument(
        '--scale', type=float, default=0.5, help='scale for gaussian weights std'
    )
    ...
    if args.mode == 'train':
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            train(args, model, train_loader, optimizer, criterion, epoch=epoch, N=args.nsample, scale=args.scale, log_file=args.result_logging_file_path+"{}_train_loss.txt".format(args.checkpoint))
