...
def pad_N_set(N_set):
    '''
    For padding sequence (currently not used)
    '''
...
def pad_collate(batch):
    ...
    segment['chunks'] = list([T1, D]) of size C
                        ...
                        T: padded timesteps
                        D: hidden dim
    segment['Q_emb'] = 5 x D (5 is hyperparameter set for training by us.)
    ...
    M = torch.max(segment_len) # max no. of chunks of a segment in the batch
    
    D = Q_emb.size()[-1] # feature dimension
    ...
    # negatives can only be sampled within group (in_segment mask) and not padded regions
    valid_negs_mask = ~(unfolded_in_segment_mask + padding_mask_grp)
    
    neg_count = valid_negs_mask.sum(-1)
    # min. negatives in a batch
    ...
    return {
        "B": B, # batch size
        "T": T, # Max. timestamp of any chunk in a batch (needed to pad all chunks in a batch)
        "M": M, # max. no. of chunks of a segment in a batch (needed to pad segment as they are variable due to VAD)
        "D": D, # feature dim (1024 in IndicASR or 768 in Vakyansh)
        "num_negs": num_negs, # no. of negatives per chunk
        "segment_len": segment_len, # no. of chunks per segment
        "batch_chunks": batch_chunks, # padded chunk initial features (from feature extractor: IndicASR or Vakyansh)
        "speech_padding_mask": speech_padding_mask, # padding mask
        "Q_emb": Q_emb, # question LABSE embeddings
        "chunk_timestep_len": chunk_timestep_len, # no. of timesteps/timeframes per chunk
        "speech_attn_mask": speech_attn_mask, # similar to padding mask in this case (initially was different when cross attention was learnt)
        "numchunks": segment_len.sum(-1), # total no. of chunks in a batch
        "unfolded_in_segment_mask":unfolded_in_segment_mask, # chunks which are in same segment
        "padding_mask_grp": padding_mask_grp, # padding mask over groups
        "sampled_neg_inds":sampled_neg_inds, # negative indices per chunk
        "valid_negs_mask":valid_negs_mask, # potential negatives
        "padding_cross_attn_mask":padding_cross_attn_mask, # avoid to attend over padded portion
    }
...
class Question_dataset_new(Dataset):
    def __init__(self, test_data_object, file_name, csv_dir, test_file, qns_file = '../../care_india/questions/questions_00-02_hindi.json', qns_emb_file = '../../care_india/questions/questions_00-02_hindi_labse_embeddings.json', transform=None):
        ...
        csv_df = pd.read_csv(csv_file_path, usecols=['Q_id', 'start_time', 'end_time'])
