...
def pad_collate(batch):
    ...
    batch_chunks = torch.from_numpy(torch.stack(collapsed_segs).detach().cpu().numpy()).squeeze()
    # print(batch_chunks.size())
    # B*N x num_Q_per_segment x D
    Q_emb = torch.from_numpy(torch.cat(Q_emb, dim=0).detach().cpu().numpy())
...
def pad_collate_test(batch):
    ...
    # seg_Q_ID = torch.cat(seg_Q_ID, dim = 0).detach().cpu().numpy()
...
class CARE_dataset(Dataset):
    ...
    def __init__(self, segs_dir, nsample, vad_text_feat_dir, D, qns_emb_file = '../../care_india/questions/questions_00-02_hindi_labse_embeddings.json', train=None, transform = None):
        ...
        for segs in seg_audio_files:
            ...
            for ind, chunk in enumerate(chunk_files):
                feat_file = chunk.replace("vad_chunks", vad_text_feat_dir).replace("wav", "npy")
                # if not os.path.exists(feat_file): continue
                feat = torch.from_numpy(np.load(feat_file))
                if feat.numel() == 0: continue
                chunks_emb_list += [feat]
...
class Test_dataset(Dataset):
    def __init__(self, segs_dir, file_name, vad_text_feat_dir, transform = None):
        self.segs_dir = segs_dir
        segments_list = []
        self.transform = transform
        ...
        for segs in seg_audio_files:
            ...
            if len(chunk_emb_list) == 0: continue
...
class Test_dataset_fixed(Dataset):
    def __init__(self, segs_dir, file_name, num_chunks, vad_text_feat_dir, transform = None):
        self.segs_dir = segs_dir
        # segments_list = []
        fixed_chunks_list = []
        self.transform = transform
...
class Question_dataset_new(Dataset):
    def __init__(self, test_data_object, file_name, csv_dir='../../care_india_data/audio_bihari_val_csv/', test_file='../../care_india/audio_bihari_features_dict_valdata_5ques.json', qns_file = '../../care_india/questions/questions_00-02_hindi.json', qns_emb_file = '../../care_india/questions/questions_00-02_hindi_labse_embeddings.json', transform=None):
        self.transform = transform
        ...
        # csv_dict = csv_df.set_index('Q_id').T.to_dict('list')
        # print(csv_df.columns)
        # qn_id_list = csv_df['Q_id'].to_list()



        # print(csv_df[csv_df.duplicated(keep=False)])
...
class Question_dataset(Dataset):
    def __init__(self, file_name, test_file='../../care_india/audio_bihari_features_dict_valdata_5ques.json', qns_file = '../../care_india/questions/questions_00-02_hindi.json', qns_emb_file = '../../care_india/questions/questions_00-02_hindi_labse_embeddings.json', transform=None):
        self.transform = transform
        test_dict = pickle.load(open(test_file, 'rb'))
        # print(test_dict.keys())
        curr_segs = test_dict[file_name]
