...
class CARE_dataset(Dataset):
    ...
    def __getitem__(self, idx): # negatives need not be matched - random x samples (chunks and qns) outside [-2k, 2k]
        k = self.k
        ...
        inds_s = random.sample(range(len(neg_s)), min(self.nsample, len(neg_s)))
        inds_t = random.sample(range(len(neg_t)), min(self.nsample, len(neg_t)))

        neg_s_sampled = [neg_s[i] for i in inds_s]
        neg_t_sampled = [neg_t[i] for i in inds_t]
