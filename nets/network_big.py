...
class Feat_Merger(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc_speech_1 = nn.Linear(768, 768)
        self.fc_speech_2 = nn.Linear(768, 768)

        self.fc_text_1 = nn.Linear(768, 768)
        self.fc_text_2 = nn.Linear(768, 768)
    ...
    def forward(self, speech = None, text = None):
        ...
        if text is not None:
            t_feat = self.fc_text_2(self.dropout_1(self.fc_text_1(text)))
            t_feat = F.normalize(t_feat, p=2, dim=-1)

        if speech is not None:
            _, h_n  = self.rnn_layer(speech)
            s_feat = self.fc_speech_2(self.dropout_3(self.fc_speech_1(self.dropout_2(h_n[0]))))
