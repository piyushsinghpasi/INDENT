...
# path for nets is added using sys in main
from nets.espnet_attn import MultiHeadedAttention
...
class Mask_and_Mean(nn.Module):
    ...
    def forward(self, convd_speech, chunk_timestep_len, max_pad):
        ...
        # print(convd_speech.size(), torch.max(chunk_timestep_len))
        convd_chunk_timestep_len = torch.floor( (chunk_timestep_len - self.kernel_size / self.stride) + 1) 
...
class Feat_Merger(nn.Module):
    ...
    def __init__(self, input_dim = 768, hidden_dim = 768, dropout = 0.3, PyramidalConv = False, kernel_size = 20, stride = 2):
        super(Feat_Merger, self).__init__()
        self.fc_speech = nn.Linear(hidden_dim, hidden_dim)
        self.fc_speech2 = nn.Linear(hidden_dim, hidden_dim)
        # self.fc_text = nn.Linear(input_dim, hidden_dim)
        
        self.aggregation_layer = SimpleConv1D(input_dim, hidden_dim, kernel_size = kernel_size, stride = stride)
    
        self.mask_and_mean_layer = Mask_and_Mean(hidden_dim = hidden_dim, kernel_size = kernel_size, stride = stride)
        ...
        # self.text_dropout = nn.Dropout(dropout)t_feat
...
class Feat_Merger(nn.Module):
    ...
    def __init__(self, input_dim = 768, hidden_dim = 768, dropout = 0.3, PyramidalConv = False, kernel_size = 20, stride = 2):
        ...
        self.gaussian_cross_attn_layer = MultiHeadedAttention(n_head = 1, n_feat = hidden_dim, dropoernel_size = kernel_size, stride = 2)
    
        self.mask_and_mean_layer = Mask_and_Mean(hidden_dim = hidden_dim, kernel_size = kernel_size)
        self.self_attn_layer = MultiHeadedAttention(n_head = 1, n_feat = hidden_dim, dropout_rate = dropout, attut_rate = dropout, attn_type='cross')
        # self.gaussian_cross_attn_layer = Gaussian_cross_attn()
    ...
    def forward(self, speech, text, speech_padding_mask, segment_len, chunk_timestep_len, max_pad, speech_attn_mask = None, padding_cross_attn_mask = None):
        ...
        t_feat = text
        # t_feat = self.fc_text_feat)
        # t_feat = self.text_norm(self.text_dropout(t_feat))
        ...
        cross_attd_chunk = self.gaussian_cross_attn_layer(s_feat, t_feat, t_feat, mask = padding_cross_attn_mask)
        # cross_attd_chunk = self.gaussian_cross_attn_layer(segment_len, self_attd_chunk.size()[1], t_feat)
        cross_attd_chunk = self.cross_attn_norm(self.dropout2(cross_attd_chunk))
